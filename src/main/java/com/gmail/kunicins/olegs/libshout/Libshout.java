...
import com.ximalaya.live.OsUtil;

public class Libshout implements AutoCloseable {
  static {
    // copy lib to /tmp and load it
    File libfile = null;
    switch (OsUtil.detectOS()) {
      case linux64:
        libfile = copyNativelib("/libshout-java-linux64.so");
        break;
      case mac64:
        libfile = copyNativelib("/libshout-java-mac64.so");
        break;
    }
    if (libfile == null) {
      throw new RuntimeException("create temp lib error!");
    }
    System.load(libfile.getAbsolutePath());
  }

  private static File copyNativelib(String lib) {
    try {
      InputStream in = Libshout.class.getResourceAsStream(lib);
      byte[] buffer = new byte[1024];
      int read = -1;
      File temp = new File("/tmp/libshout-java.so");
      temp.createNewFile();
